...
Config_Row = 1; %Sets row of design input spreadsheet being evaluated (if using Open VSP, leave at 1)
...
%Set aircraft initial wing planform area
...
TO_REQ_Label = 'Takeoff Dist';

Beta = 1; %Weight fraction at time of point performance (W/Wo)
h = 0; %Altitude for point performance requirement (ft)
Throttle = 'PROP'; %Defines if jet afterburner thrust ('AB'), jet mil thrust ('MIL'), or propellor power available ('PROP') is used in performance analysis
...
S_TO = 805; %Takeoff groundroll distance requirement (ft)
...
%Landing Req Inputs
    Beta = 1; %Weight fraction at time of point performance (W/Wo)
    h = 0; %Altitude for point performance requirement (ft)
    CL_max_land = 1.3; %Max coefficient of lift in landing configuration (including flaps)
    Roll_Fric = 0.3; %Coefficient of braking & rolling friction based on runway surface
    CDx = 0; %Defines added parasite drag coefficient value for landing gear (see Raymer Table 12.6 for landing gear estimates)
    S_Land = 520; %Landing groundroll distance requirement (ft)
...
%Stall Velocity Req Inputs
    h = 0; %Altitude for point performance requirement (ft)
    CL_max_stall = 1.4; %Max coefficient of lift for stall (SLUF)
    V_stall_req = 50; %Desired stall velocity in knots (nautical miles per hour)
...
%REQ1 Req Inputs
    Beta = 0.5; %Weight fraction at time of point performance (W/Wo)
    h = 8000; %Altitude for point performance requirement (ft)
    M = 0.18; %Mach for point performance requirement
...
REQ2_Label = 'Absolute Ceiling';

%REQ2 Req Inputs
    Beta = 0.75; %Weight fraction at time of point performance (W/Wo)
    h = 15000; %Altitude for point performance requirement (ft)
    M = 0.17; %Mach for point performance requirement
    n = 1; %Load factor for point performance requirement
    Throttle = 'PROP'; %Defines if jet afterburner thrust ('AB'), jet mil thrust ('MIL'), or propellor power available ('PROP') is used in performance analysis
    P_s = 0; %Rate of Climb req (ft/s) or if point performance requirement must be sustained (P_s = 0) or accomplished with remaining excess power available (P_s > 0) or at a deficit (P_s < 0)
    Accel = 0; %Defines any acceleration requirement (ft/s^2). Typically zero for most point performance requirements.
...
REQ3_Label = 'Service Ceiling';

%REQ3 Req Inputs
    Beta = 0.75; %Weight fraction at time of point performance (W/Wo)
    h = 14200; %Altitude for point performance requirement (ft)
    M = 0.17; %Mach for point performance requirement
    n = 1; %Load factor for point performance requirement
    Throttle = 'PROP'; %Defines if jet afterburner thrust ('AB'), jet mil thrust ('MIL'), or propellor power available ('PROP') is used in performance analysis
...
%% REQ #4: SUSTAIN TURN
REQ4_Label = 'Sustain Turn';

%REQ4 Req Inputs
    Beta = 0.875; %Weight fraction at time of point performance (W/Wo)
    h = 8000; %Altitude for point performance requirement (ft)
    M = 0.17; %Mach for point performance requirement
    n = 1.5; %Load factor for point performance requirement
    Throttle = 'PROP'; %Defines if jet afterburner thrust ('AB'), jet mil thrust ('MIL'), or propellor power available ('PROP') is used in performance analysis
    P_s = 0; %Rate of Climb req (ft/s) or if point performance requirement must be sustained (P_s = 0) or accomplished with remaining excess power available (P_s > 0) or at a deficit (P_s < 0)
    Accel = 0; %Defines any acceleration requirement (ft/s^2). Typically zero for most point performance requirements.
    CDx = 0; %Defines added parasite drag coefficient value not modeled in clean aerodynamics drag polar values due to configuration (landing gear, flaps) or external stores (attached sensors, fuel tanks, munitions, etc.)
...
if strcmp(PropType,'PROP_Fuel')|strcmp(PropType,'PROP_Electric')
    ...
    % Desired Design Point (PROP) based on review of constraint diagram
    P_W_1 = 0.1;
    W_S_1 = 8;
    ...
    % Desired Design Point (JET) based on review of constraint diagram plot
