function [Cruise_W_fraction,CRUISE_DATA,msgs] = MSN_SEG_Cruise(Cruise_Alt_Start,R_cruise,Cruise_M,CDx_Cruise,W_start,Constants,Propulsion_Input,DragPolar_Model,WaveDrag_Data,msgs)
    ...
    Propulsion(Cruise_Alt_Start,Cruise_M,Config_Row,Propulsion_Input,msgs);
        ...
        if strcmp(PropType,'PROP_Fuel')||strcmp(PropType,'PROP_Electric')||strcmp(PropType,'PROP_Turbocharged')||strcmp(PropType,'Turboprop')
            ...
            if PA_max > Power_Req %Check to see if max power available is greater than power required during cruise
                Cruise_W_fraction = exp(-R_cruise*SFC/(Propulsion_Input.Prop_eff(Config_Row)*(CL/CD))); %Note: Equation different from Raymer 6.15 due to conversion from hp to ft*lb/s already accomplished in propulsion.m function for SFC.
            else
                Cruise_W_fraction = exp(-R_cruise*SFC/(Propulsion_Input.Prop_eff(Config_Row)*(CL/CD))); %Note: Equation different from Raymer 6.15 due to conversion from hp to ft*lb/s already accomplished in propulsion.m function for SFC.
            ...
            CRUISE_DATA= table(Cruise_Alt_Start,Cruise_M,Cruise_Vel,CL,CD,CDo_msn,k1_msn,k2_msn,D,PA_max,Power_Req,SFC);
            ...
            if D > TA_mil_max & D <= TA_AB_max %Does drag produced require afterburner to have sufficient thrust?
                Cruise_W_fraction = exp(-R_cruise*TSFC_AB/(Cruise_Vel*(CL/CD))); 
            elseif D <= TA_mil_max %Only require mil power for drag during cruise
                Cruise_W_fraction = exp(-R_cruise*TSFC_mil/(Cruise_Vel*(CL/CD)));
            elseif D > max(TA_AB_max,TA_mil_max) %Drag is greater than max of thrust available (AB or mil)
                Cruise_W_fraction = exp(-R_cruise*TSFC_AB/(Cruise_Vel*(CL/CD))); %Output anyway to ensure data is written for analysis only
                msgs.warnings{end+1} = 'Thrust insufficient for cruise based on drag!';
                warning(msgs.warnings{end,:})
            end
            %Record Cruise Performance Data
            CRUISE_DATA= table(Cruise_Alt_Start,Cruise_M,Cruise_Vel,CL,CD,CDo_msn,k1_msn,k2_msn,D,TA_mil_max,TA_AB_max,TSFC_mil,TSFC_AB,PA_max,SFC);
        end
